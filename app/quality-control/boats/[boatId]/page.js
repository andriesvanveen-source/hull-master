              ...
              {boat.areas.map((area) => (
                <Fragment key={area}>
                  ...
                  <tr className={styles.draftRow}>
                    <td></td>
                    <td></td>
                    <td><input value={drafts[area]?.item || ""} onChange={(event) => setDraft(area, "item", event.target.value)} list={`qc-items-${normalize(area)}`} placeholder="Select or type item/part/subcomponent" aria-label={`${area} item, part or subcomponent`} /><datalist id={`qc-items-${normalize(area)}`}>{uniqueItemSuggestions(catalog.filter((entry) => normalize(entry.area) === normalize(area))).sort((a, b) => a.item.localeCompare(b.item)).map((entry) => <option key={normalize(entry.item)} value={entry.item} />)}</datalist></td>
                    <td><input value={drafts[area]?.failure || ""} onChange={(event) => setDraft(area, "failure", event.target.value)} list={`qc-failures-${normalize(area)}`} placeholder="Select or type failure mode" aria-label={`${area} failure mode`} /><datalist id={`qc-failures-${normalize(area)}`}>{uniqueValues(catalog.filter((entry) => normalize(entry.area) === normalize(area) && normalize(entry.item) === normalize(drafts[area]?.item)), "issue").map((failure) => <option key={normalize(failure)} value={failure} />)}</datalist></td>
                    <td><input value={drafts[area]?.description || ""} onChange={(event) => setDraft(area, "description", event.target.value)} list={`qc-descriptions-${normalize(area)}`} placeholder="Select or type defect description" aria-label={`${area} defect description`} /><datalist id={`qc-descriptions-${normalize(area)}`}>{uniqueValues(catalog.filter((entry) => normalize(entry.area) === normalize(area) && normalize(entry.item) === normalize(drafts[area]?.item) && normalize(entry.issue) === normalize(drafts[area]?.failure)), "defect").map((description) => <option key={normalize(description)} value={description} />)}</datalist></td>

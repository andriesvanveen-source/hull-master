...
function mapRemoteBoat(boat) {
  ...
  return {
    ...
    auditType: /_HO_Audit$/i.test(boat.name || "") ? "HO" : "QC3",
...
export async function syncQualityBoat(boat) {
  ...
  const areasToSync = boat.syncScopeVersion === 1
    ? (boat.areas || []).filter((areaName) => (boat.pendingAreaNames || []).includes(areaName))
    : (boat.areas || []);
  const areas = areasToSync.map((areaName) => ({ boat_id: boat.id, area_name: areaName, inspector: boat.areaInspectors?.[areaName] || "", sort_order: (boat.areas || []).indexOf(areaName), updated_at: updatedAt }));
  if (areas.length) await upsertInBatches("quality_control_areas", areas, { onConflict: "boat_id,area_name" });
  const defectsToSync = boat.syncScopeVersion === 1
    ? (boat.defects || []).filter((defect) => (boat.pendingDefectIds || []).includes(defect.id))
    : (boat.defects || []);
  const defects = defectsToSync.map((defect) => ({ id: defect.id, boat_id: boat.id, area_name: defect.area, item: defect.item, failure_mode: defect.failure, description: defect.description, code: defect.code ? Number(defect.code) : null, discipline: defect.discipline || "", concern: Boolean(defect.concern), repaired_by: defect.repairedBy || "", repaired_date: defect.repairedDate || "", team_leader_check: defect.teamLeaderCheck || "", qc_rwk: defect.qcRwk || "", qc_acc: defect.qcAcc || "", created_at: defect.createdAt || updatedAt, updated_at: defect.updatedAt || updatedAt }));

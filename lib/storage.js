...
import { DEFAULT_COMMISSIONING_ENGINEER } from "./constants";
...
function mapBoat(row) {
  ...
  return {
    id: row.id,
    name: row.name || "",
    commissioningEngineer: row.commissioning_engineer || DEFAULT_COMMISSIONING_ENGINEER,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
...
export async function loadState() {
  ...
  const { data, error } = await supabase
    .from("boats")
    .select("id,name,commissioning_engineer,created_at,updated_at,defects(id,text,discipline,area,created_at,updated_at)")
...
export async function createBoat(name, commissioningEngineer = DEFAULT_COMMISSIONING_ENGINEER) {
  ensureSupabase();

  const normalizedName = name.trim().toUpperCase();
  const normalizedEngineer = commissioningEngineer.trim() || DEFAULT_COMMISSIONING_ENGINEER;
  const { data, error } = await supabase
    .from("boats")
    .insert({
      name: normalizedName,

...
export async function loadState() {
  ...
  const { data, error } = await supabase
    .from("boats")
    .select("id,name,created_at,updated_at,defects(id,text,discipline,area,photo_url,created_at,updated_at)")
...
export async function duplicateBoat(sourceBoat, name) {
  ...
  const { data, error } = await supabase
    .from("defects")
    .insert(defects)
    .select("id,text,discipline,area,photo_url,created_at,updated_at");
...
export async function updateDefectRecord(defectId, values) {
  ...
  if (Object.prototype.hasOwnProperty.call(values, "photoUrl")) {
    patch.photo_url = values.photoUrl || null;
  }
...
export async function uploadDefectPhoto({ boatName, defectId, file }) {
  ensureSupabase();

  const extension = file.name.split(".").pop()?.toLowerCase() || "jpg";
  const safeBoatName = boatName.replace(/[^a-z0-9-]+/gi, "-").replace(/^-|-$/g, "") || "boat";
  const filePath = `${safeBoatName}/${defectId}-${Date.now()}.${extension}`;
  ...
  await updateDefectRecord(defectId, { photoUrl });
  return photoUrl;
}

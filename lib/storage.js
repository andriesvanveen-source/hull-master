...
import { BOAT_MODELS, DEFAULT_BOAT_AREAS, GENERAL_AREA, getCommonDefectAreaNames, normalizeBoatArea, orderBoatAreas } from "./constants";
...
function mapDefect(row) {
  return {
    ...
    area: normalizeBoatArea(row.area),
...
export async function createDefect({ id, boatId, text, discipline, area }) {
  ensureSupabase();
  ...
  if (id) defectRecord.id = id;

  const { data, error } = await supabase
    .from("defects")
    .upsert(defectRecord, { onConflict: "id" })
    .select("id,text,discipline,area,created_at,updated_at")
    .single();

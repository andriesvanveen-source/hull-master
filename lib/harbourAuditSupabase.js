...
export async function syncHarbourAudit(audit) {
  ...
  // Remove server defects that the user removed locally, but only after every
  // remaining defect and photo has been uploaded successfully.
  const currentDefectIds = new Set((audit.defects || []).map((defect) => defect.id));
  const { data: storedDefects, error: storedDefectsError } = await supabase
    .from("harbour_audit_defects")
    .select("id,harbour_audit_photos(image_path)")
    .eq("audit_id", audit.id);
  if (storedDefectsError) throw storedDefectsError;
  const staleDefects = (storedDefects || []).filter((defect) => !currentDefectIds.has(defect.id));
  if (staleDefects.length) {
    const stalePaths = staleDefects.flatMap((defect) => (
    ...
    const { error: staleDefectsError } = await supabase
      .from("harbour_audit_defects")
      .delete()
      .in("id", staleDefects.map((defect) => defect.id));
    if (staleDefectsError) throw staleDefectsError;
  }
}

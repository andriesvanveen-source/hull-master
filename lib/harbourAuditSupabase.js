...
export async function loadSharedHarbourAudits() {
  ...
  let { data, error } = await supabase
    .from("harbour_audits")
    .select("id,title,auditor,created_label,created_at,updated_at,deleted_at,harbour_audit_defects(id,area_name,description,created_at,updated_at,harbour_audit_photos(id,file_name,image_path,created_at))")
    .order("updated_at", { ascending: false });

  // Keep existing installations operational while the non-destructive
  // deletion-marker migration is waiting to be run.
  if (error && /deleted_at/i.test(error.message || "")) {
    const fallback = await supabase
      .from("harbour_audits")
      .select("id,title,auditor,created_label,created_at,updated_at,harbour_audit_defects(id,area_name,description,created_at,updated_at,harbour_audit_photos(id,file_name,image_path,created_at))")
      .order("updated_at", { ascending: false });
    data = (fallback.data || []).map((audit) => ({ ...audit, deleted_at: null }));
    error = fallback.error;
  }

  if (error) throw error;

  const deletedAuditIds = (data || []).filter((audit) => audit.deleted_at).map((audit) => audit.id);
  const audits = await Promise.all((data || []).filter((audit) => !audit.deleted_at).map(async (audit) => ({
    id: audit.id,
    title: audit.title,
    ...
    pendingDefectIds: [],
    deletedDefectIds: [],
    defects: await Promise.all([...(audit.harbour_audit_defects || [])]
      .sort((left, right) => {
...
export async function syncHarbourAudit(audit) {
  ...
  const defectsToSync = audit.syncScopeVersion === 1
    ? (audit.defects || []).filter((defect) => (audit.pendingDefectIds || []).includes(defect.id))
    : (audit.defects || []);
  for (const defect of defectsToSync) {
    const { error: defectError } = await supabase.from("harbour_audit_defects").upsert({
      id: defect.id,
      audit_id: audit.id,
      area_name: defect.area || "General",
      description: defect.description,
  ...
  // Delete only defects explicitly removed by a user. Never infer deletions
  // from another device's potentially stale local audit snapshot.
  const deletedDefectIds = [...new Set(audit.deletedDefectIds || [])];
  if (deletedDefectIds.length) {
    const { data: deletedDefects, error: deletedDefectsError } = await supabase
      .from("harbour_audit_defects")
      .select("id,harbour_audit_photos(image_path)")
      .eq("audit_id", audit.id)
      .in("id", deletedDefectIds);
    if (deletedDefectsError) throw deletedDefectsError;
    const stalePaths = (deletedDefects || []).flatMap((defect) => (
      defect.harbour_audit_photos || []
    ).map((photo) => photo.image_path).filter(Boolean));
    if (stalePaths.length) {
      const { error: removeError } = await supabase.storage.from(PHOTO_BUCKET).remove(stalePaths);
      if (removeError) throw removeError;
    }
    const { error: deletedRowsError } = await supabase
      .from("harbour_audit_defects")
      .delete()
      .eq("audit_id", audit.id)
      .in("id", deletedDefectIds);
...
export async function deleteSharedHarbourAudit(audit) {
  ...
  const { error } = await supabase.from("harbour_audits").upsert({
    id: audit.id,
    title: audit.title || "Deleted audit",
    auditor: audit.auditor || "",
    created_label: audit.createdAt || "",
  ...
  const { error: defectError } = await supabase.from("harbour_audit_defects").delete().eq("audit_id", audit.id);
  if (defectError) throw defectError;
}

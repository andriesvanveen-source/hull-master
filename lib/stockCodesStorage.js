...
async function loadRemoteStockItems() {
  const fields = "id,stock_code,description,search_terms,category_id,image_path,image_url,order_count,created_at,stock_categories(name)";
  const response = await supabase.from("stock_items").select(fields).order("stock_code");
  if (!response.error || (response.error.code !== "42703" && !response.error.message?.includes("order_count"))) return response;
  return supabase
    .from("stock_items")
...
export async function syncStockItem(item) {
  ...
  if (Number(item.orderCount) > 0) payload.order_count = Number(item.orderCount);
  ...
  if (error) throw error;
  return { ...mapItem(data), orderCount: Number(item.orderCount) || 0 };
}

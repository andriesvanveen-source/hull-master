...
export const DEFAULT_STOCK_CATEGORIES = [
  ...
  { id: "10000000-0000-4000-8000-000000000004", name: "Loose items/Skipper's pack" },
...
function ensureDefaultCategories(categories) {
  const combinedName = "Loose items/Skipper's pack";
  const renamed = categories.map((category) => {
    const name = category.name.toLowerCase().replace(/\s+/g, " ").trim();
    return name === "skipper's pack" || name === "loose items" || name === "loose items/skipper's pack"
      ? { ...category, name: combinedName }
      : category;
  });
  const uniqueCategories = [...new Map(renamed.map((category) => [category.name.toLowerCase(), category])).values()];
  const names = new Set(uniqueCategories.map((category) => category.name.toLowerCase()));
  return [
    ...uniqueCategories,

...
export const DEFAULT_BOAT_AREAS = [...COMMON_DEFECT_AREAS];
...
const CANONICAL_AREA_NAMES = {
  "stbd engine": "Stbd Engine Room",
  "starboard engine": "Stbd Engine Room",
  "starboard engine room": "Stbd Engine Room",
  "port engine": "Port Engine Room",
  "port engine area": "Port Engine Room",
  "port engine room": "Port Engine Room"
};

export function normalizeBoatArea(area) {
  const normalizedArea = String(area || "").trim();
  return CANONICAL_AREA_NAMES[normalizedArea.toLowerCase()] || normalizedArea;
}
...
export function getCommonDefectAreaNames(area) {
  const normalizedArea = normalizeBoatArea(area);

  if (!normalizedArea) {
    return [];
  }

  const aliases = Object.entries(COMMON_DEFECT_AREA_ALIASES).flatMap(([legacyArea, canonicalAreas]) => {
    const relatedNames = [legacyArea, ...canonicalAreas];
    return relatedNames.some((name) => name.toLowerCase() === normalizedArea.toLowerCase()) ? relatedNames : [];
  });

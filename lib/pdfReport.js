...
export async function exportBoatReport(boat, options = {}) {
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Discipline Breakdown", 118, 37);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  const disciplineSummary = DISCIPLINES
    .map((discipline) => `${discipline}: ${disciplineCounts[discipline]}`)
    .join("  |  ");
  const disciplineLines = doc.splitTextToSize(disciplineSummary, pageWidth - 130);
  doc.text(disciplineLines, 118, 43);

  if (showRepeatDefects && repeatedDefectTexts.size > 0) {
    const repeatKeyY = Math.max(57, 43 + disciplineLines.length * 4.2);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.5);
    doc.text("Repeat Key", 118, repeatKeyY);
    doc.setFillColor(...repeatFillColor);
    doc.rect(118, repeatKeyY + 3, 7, 5, "F");
    doc.setDrawColor(183, 151, 60);
    doc.rect(118, repeatKeyY + 3, 7, 5, "S");
    doc.setFont("helvetica", "normal");
    doc.text(`Repeated from ${previousBoat.name}`, 128, repeatKeyY + 7);
  }

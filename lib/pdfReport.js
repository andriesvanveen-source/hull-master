"use client";

import { normalizeDefectText } from "./commonDefects";
import { DISCIPLINES, getBoatAreas, isGeneralArea } from "./constants";

const DISCIPLINE_CHART_COLORS = {
  PLUM: [17, 72, 111],
  ELEC: [244, 184, 72],
  MECH: [24, 115, 117],
  FA: [239, 123, 105],
  DECK: [93, 74, 140],
  CARP: [101, 177, 213],
  FIN: [75, 87, 94]
};

function createDisciplinePieChart(disciplineCounts) {
  const canvas = document.createElement("canvas");
  const size = 512;
  const center = size / 2;
  const radius = size * 0.46;
  canvas.width = size;
  canvas.height = size;
  const context = canvas.getContext("2d");
  const total = DISCIPLINES.reduce((sum, discipline) => sum + disciplineCounts[discipline], 0);

  if (total === 0) {
    context.fillStyle = "rgb(221, 226, 229)";
    context.beginPath();
    context.arc(center, center, radius, 0, Math.PI * 2);
    context.fill();
    return canvas.toDataURL("image/png");
  }

  let startAngle = -Math.PI / 2;
  DISCIPLINES.forEach((discipline) => {
    const value = disciplineCounts[discipline];
    if (!value) return;
    const endAngle = startAngle + (value / total) * Math.PI * 2;
    context.fillStyle = `rgb(${DISCIPLINE_CHART_COLORS[discipline].join(", ")})`;
    context.beginPath();
    context.moveTo(center, center);
    context.arc(center, center, radius, startAngle, endAngle);
    context.closePath();
    context.fill();
    startAngle = endAngle;
  });

  return canvas.toDataURL("image/png");
}

function drawDisciplineBreakdown(doc, disciplineCounts) {
  doc.setTextColor(20, 28, 34);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9.5);
  doc.text("Discipline Breakdown", 118, 35);
  doc.addImage(createDisciplinePieChart(disciplineCounts), "PNG", 120.5, 38.5, 23, 23, undefined, "FAST");

  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.2);
  DISCIPLINES.forEach((discipline, index) => {
    const column = index < 4 ? 0 : 1;
    const row = column === 0 ? index : index - 4;
    const x = 148 + column * 25;
    const y = 40 + row * 5.2;
    doc.setFillColor(...DISCIPLINE_CHART_COLORS[discipline]);
    doc.rect(x, y - 2.7, 3, 3, "F");
    doc.setTextColor(20, 28, 34);
    doc.text(`${discipline}: ${disciplineCounts[discipline]}`, x + 4.5, y);
  });
}

function getAreaRows(boat, areas) {
  return areas.reduce((acc, area) => {
    acc[area] = boat.defects.filter((defect) => defect.area === area);
    return acc;
  }, {});
}

function getRowNumbers(boat, areas) {
  let current = 1;
  const numbers = {};

  areas.filter((area) => !isGeneralArea(area)).forEach((area) => {
    boat.defects
      .filter((defect) => defect.area === area && defect.text.trim())
      .forEach((defect) => {
        numbers[defect.id] = current;
        current += 1;
      });
  });

  return numbers;
}

export async function exportBoatReport(boat, options = {}) {
  const createdAt = new Date();
  const safeBoatName = boat.name.replace(/[^a-z0-9-]+/gi, "-").replace(/^-|-$/g, "") || "hull";
  const commissioningEngineer = boat.commissioningEngineer || "";
  const boatAreas = getBoatAreas(boat);
  const requestedAreas = Array.isArray(options.areas) ? new Set(options.areas) : null;
  const areas = boatAreas.filter((area) => (
    (!requestedAreas || requestedAreas.has(area))
    && boat.defects.some((defect) => defect.area === area && defect.text.trim())
  ));
  const exportedAreas = new Set(areas);
  const exportedDefects = boat.defects.filter((defect) => (
    exportedAreas.has(defect.area) && defect.text.trim()
  ));
  const defectCount = boat.defects.filter((defect) => (
    exportedAreas.has(defect.area)
    && !isGeneralArea(defect.area)
    && defect.text.trim()
  )).length;
  const disciplineCounts = Object.fromEntries(DISCIPLINES.map((discipline) => [
    discipline,
    exportedDefects.filter((defect) => String(defect.discipline || "").trim().toUpperCase() === discipline).length
  ]));

  if (areas.length === 0) {
    throw new Error("No logged defects were found in the audited areas.");
  }

  const { jsPDF } = await import("jspdf");
  const { default: autoTable } = await import("jspdf-autotable");
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  const generatedAt = createdAt.toLocaleString();
  const tableRows = [];
  const areaRows = getAreaRows(boat, areas);
  const rowNumbers = getRowNumbers(boat, areas);
  const previousBoat = options.previousBoat || null;
  const showRepeatDefects = Boolean(options.showRepeatDefects && previousBoat);
  const repeatedDefectTexts = new Set(
    showRepeatDefects
      ? previousBoat.defects
        .map((defect) => normalizeDefectText(defect.text))
        .filter(Boolean)
      : []
  );
  const repeatedDefectCount = showRepeatDefects
    ? exportedDefects.filter((defect) => repeatedDefectTexts.has(normalizeDefectText(defect.text))).length
    : 0;
  const repeatFillColor = [255, 242, 184];
  const blankRowsByArea = options.blankRowsByArea || {};

  function rowCell(content, isRepeatDefect, styles = {}) {
    if (!isRepeatDefect) {
      return content;
    }

    return {
      content,
      styles: {
        fillColor: repeatFillColor,
        ...styles
      }
    };
  }

  areas.forEach((area) => {
    const defects = areaRows[area] || [];

    tableRows.push([
      {
        content: area,
        colSpan: 6,
        styles: {
          fillColor: [223, 232, 227],
          fontStyle: "bold",
          fontSize: 9.5,
          textColor: [23, 33, 30],
          halign: "left"
        }
      },
      "",
      {
        content: "",
        colSpan: 2,
        styles: { fillColor: [230, 238, 240] }
      }
    ]);

    if (defects.length === 0) {
      tableRows.push([
        {
          content: "- no defects logged -",
          colSpan: 6,
          styles: {
            fontStyle: "italic",
            halign: "center",
            textColor: [110, 110, 110]
          }
        },
        "",
        { content: "", colSpan: 2 }
      ]);
    }

    defects.forEach((defect) => {
      const isRepeatDefect = repeatedDefectTexts.has(normalizeDefectText(defect.text));

      tableRows.push([
        rowCell(isGeneralArea(area) ? "" : String(rowNumbers[defect.id] || ""), isRepeatDefect),
        rowCell(defect.text, isRepeatDefect),
        rowCell(defect.discipline || "-", isRepeatDefect),
        rowCell("", isRepeatDefect),
        rowCell("", isRepeatDefect),
        rowCell("", isRepeatDefect),
        "",
        rowCell("", isRepeatDefect),
        rowCell("", isRepeatDefect)
      ]);
    });

    for (let index = 0; index < (blankRowsByArea[area] || 0); index += 1) {
      tableRows.push(["", "", "", "", "", "", "", "", ""]);
    }
  });

  doc.setFillColor(11, 45, 73);
  doc.rect(0, 0, pageWidth, 22, "F");
  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(18);
  doc.text("Yacht Commissioning Audit", 12, 13);
  doc.setFontSize(9);
  doc.text("Hull Master", pageWidth - 12, 13, { align: "right" });

  doc.setTextColor(20, 28, 34);
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.roundedRect(12, 29, pageWidth - 24, 40, 2, 2);
  doc.setFont("helvetica", "bold");
  doc.text("Hull Number", 18, 37);
  doc.text("Report Date", 68, 37);
  doc.text("Logged Defects", 18, 49);
  doc.text("Report Type", 68, 49);
  doc.text("Commissioning Engineer", 18, 61);
  doc.setFont("helvetica", "normal");
  doc.text(boat.name, 18, 43);
  doc.text(generatedAt, 68, 43);
  doc.text(String(defectCount), 18, 55);
  doc.text("Commissioning Defect Audit", 68, 55);
  doc.text(commissioningEngineer, 18, 67);

  drawDisciplineBreakdown(doc, disciplineCounts);

  if (repeatedDefectCount > 0) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7.2);
    doc.text("Repeat:", 148, 64.8);
    doc.setFillColor(...repeatFillColor);
    doc.rect(160, 62.2, 4, 3.5, "F");
    doc.setDrawColor(183, 151, 60);
    doc.rect(160, 62.2, 4, 3.5, "S");
    doc.setFont("helvetica", "normal");
    doc.text(`${repeatedDefectCount} repeat${repeatedDefectCount === 1 ? "" : "s"} from ${previousBoat.name}`, 166, 64.8);
  }

  autoTable(doc, {
    startY: 78,
    head: [[
      { content: "#", rowSpan: 2 },
      { content: "Defect", rowSpan: 2 },
      { content: "Discipline", rowSpan: 2 },
      { content: "Team Member / Date", rowSpan: 2 },
      { content: "Team Leader", rowSpan: 2 },
      { content: "Commissioning Engineer", rowSpan: 2 },
      { content: "", rowSpan: 2 },
      { content: "Callback", colSpan: 2 }
    ], [
      "Callback Date",
      "Team Leader Re-sign"
    ]],
    body: tableRows,
    theme: "grid",
    rowPageBreak: "avoid",
    tableLineColor: [130, 144, 139],
    tableLineWidth: 0.25,
    margin: { left: 12, right: 12, top: 30, bottom: 16 },
    styles: {
      font: "helvetica",
      fontSize: 6.8,
      cellPadding: 1.5,
      lineColor: [130, 144, 139],
      lineWidth: 0.25,
      minCellHeight: 8,
      textColor: [17, 17, 17],
      valign: "middle"
    },
    headStyles: {
      fillColor: [47, 58, 56],
      textColor: [255, 255, 255],
      fontStyle: "bold",
      halign: "center",
      lineColor: [130, 144, 139],
      lineWidth: 0.25
    },
    columnStyles: {
      0: { cellWidth: 7, halign: "center" },
      1: { cellWidth: 43.5 },
      2: { cellWidth: 14, halign: "center" },
      3: { cellWidth: 27 },
      4: { cellWidth: 21 },
      5: { cellWidth: 23 },
      6: { cellWidth: 2.5, cellPadding: 0 },
      7: { cellWidth: 21, halign: "center" },
      8: { cellWidth: 27 }
    },
    didParseCell: (data) => {
      if (data.column.index === 6) {
        data.cell.styles.fillColor = [255, 255, 255];
        data.cell.styles.lineWidth = 0;
      }

      if (data.section === "head" && data.column.index >= 7) {
        data.cell.styles.fillColor = [30, 82, 96];
      }
    },
    didDrawPage: (data) => {
      const pageNumber = doc.internal.getNumberOfPages();

      if (pageNumber > 1) {
        doc.setFillColor(11, 45, 73);
        doc.rect(0, 0, pageWidth, 16, "F");
        doc.setTextColor(255, 255, 255);
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
        doc.text(`Yacht Commissioning Audit - Hull ${boat.name}`, 12, 10);
      }

      doc.setDrawColor(130, 144, 139);
      doc.line(12, pageHeight - 11, pageWidth - 12, pageHeight - 11);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8);
      doc.setTextColor(95, 103, 99);
      doc.text(`Generated ${generatedAt}`, 12, pageHeight - 6);
      doc.text(`Page ${data.pageNumber}`, pageWidth - 12, pageHeight - 6, { align: "right" });
    }
  });

  doc.save(`${safeBoatName}-commissioning-audit.pdf`);
  return createdAt;
}

...
function drawDisciplineBreakdown(doc, disciplineCounts) {
  ...
  if (total === 0) {
    ...
    DISCIPLINES.forEach((discipline) => {
      ...
      const segmentCount = Math.max(1, Math.ceil((endAngle - startAngle) / (Math.PI / 24)));
      doc.setFillColor(...DISCIPLINE_CHART_COLORS[discipline]);
      for (let segment = 0; segment < segmentCount; segment += 1) {
        const angleA = startAngle + ((endAngle - startAngle) * segment) / segmentCount;
        const angleB = startAngle + ((endAngle - startAngle) * (segment + 1)) / segmentCount;
        doc.triangle(
          centerX,
          centerY,

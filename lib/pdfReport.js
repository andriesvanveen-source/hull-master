...
import { BOAT_AREAS, DEFAULT_COMMISSIONING_ENGINEER } from "./constants";

function getAreaRows(boat) {
...
export async function exportBoatReport(boat) {
  ...
  const commissioningEngineer = boat.commissioningEngineer || DEFAULT_COMMISSIONING_ENGINEER;
  ...
  const tableRows = [];
  const areaRows = getAreaRows(boat);
  const rowNumbers = getRowNumbers(boat);
